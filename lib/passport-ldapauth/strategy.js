...
var handleAuthentication = function(req, options) {
  var username, password, ldap, self = this;
  options || (options = {});
   ...
   */
  var messages = {
    '501' : options.networkError || 'Failed to connect to LDAP Auth Provider',
  ...
  ldap.on('error', function(err) {
    ...
    self.fail({message: options.networkError || (err && err.message)}, 501);
  });
